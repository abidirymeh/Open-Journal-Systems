...
{   private const SECTION_TYPE_MESSAGE = 'message';
    const SUBMISSION_FILE_REVIEW = 1;
    ...
    protected function showWizard(array $args, Request $request, Submission $submission): void
    {
        ...
        $anonymeFilesListPanel = $this->getAnonymeFilesListPanel($request, $submission, $genres);
$components[$anonymeFilesListPanel['id']] = $anonymeFilesListPanel;
     ...
     */
    /**
 * ************************************************************************************************Get the state for the AnonymeFilesListPanel component
 */
...
protected function getAnonymeFilesListPanel(Request $request, Submission $submission, array $genres): array {
    // On ne récupère plus les fichiers anonymes pour ne pas les afficher ici

    $genres = array_values(array_filter($genres, fn($genre) => !$genre->getDependent()));

    $form = new PKPSubmissionFileForm(
        $this->getSubmissionFilesApiUrl($request, $submission->getId(), SubmissionFile::SUBMISSION_FILE_ANONYMOUS),
        $genres,
        SubmissionFile::SUBMISSION_FILE_ANONYMOUS
    );

    return [
        'id' => 'anonymeFiles',
        'apiUrl' => $this->getSubmissionFilesApiUrl($request, $submission->getId(), SubmissionFile::SUBMISSION_FILE_ANONYMOUS),
        'fileStage' => SubmissionFile::SUBMISSION_FILE_ANONYMOUS,
        // On garde uniquement le formulaire d'upload
        'form' => $form->getConfig(),
        ...
        'title' => __('submission.anonymousFiles'),
        'canUpload' => true,
        'options' => [
            'maxFilesize' => Application::getIntMaxFileMBs(),
            'dropzoneDictDefaultMessage' => __('form.dropzone.dictDefaultMessage'),
        ...
        // NE PAS envoyer la liste 'items' => [...] pour ne pas afficher les fichiers existants
    ];
}
...
public function displayAnonymousUploadForm(Request $request, array $args): void
{
    $submissionId = (int) $request->getUserVar('submissionId');
    $submission = Repo::submission()->get($submissionId);
    $templateMgr = TemplateManager::getManager($request);

    // Pas de liste des fichiers ici, juste un formulaire simple
    $templateMgr->assign([
    ...
    $templateMgr->display('submission/anonymousUploadForm.tpl');
}

protected function getAnonymeFilesStep(Request $request, Submission $submission, Publication $publication, array $locales, string $publicationApiUrl): array
{
    return [
        'id' => 'anonymeUpload',
        'name' => __('submission.upload.anonymousUpload'),
        'reviewName' => __('submission.anonymousFiles'),
        'sections' => [
            [
                'id' => 'anonymeFiles',
                'name' => __('submission.upload.anonymousUpload'),
                'type' => self::SECTION_TYPE_FILES,
                'description' => __('submission.upload.anonymousFileDescription'),
                'fileStage' => SubmissionFile::SUBMISSION_FILE_ANONYMOUS,
                'canUpload' => true,
            ],
        ],
        // Supprimer la ligne ci-dessous pour ne pas afficher la liste des fichiers
    ...
    protected function getSteps(Request $request, Submission $submission, Publication $publication, array $locales, array $sections, LazyCollection $categories): array
    {
        ...
        $steps[] = $this->getAnonymeFilesStep($request, $submission, $publication, $locales, $publicationApiUrl);
     ...
     */
    protected function getSubmissionFilesApiUrl(Request $request, int $submissionId, int $fileStage = null): string {
    $url = $request->getDispatcher()->url($request, Application::ROUTE_API, $request->getContext()->getPath(), 'submissions/' . $submissionId . '/files');
    if ($fileStage !== null) {
        $url .= '?fileStage=' . $fileStage;
    }
    return $url;
}
